...
import { MdDashboard, MdPeople, MdHotel, MdDescription, MdSend, MdHealthAndSafety } from "react-icons/md";
        ...
        <aside className="w-72 h-screen bg-slate-900 text-slate-400 flex flex-col justify-between p-6 border-r border-slate-800 sticky top-0 shrink-0 z-40">
            <div className="flex flex-col gap-8 w-full">
                ...
                <nav className="flex flex-col gap-1.5 w-full">
                    {tabs.map((tab) => {
                        const isTabActive = location.pathname.startsWith(`/doctor/${tab.id}`);

                        return (
                            <NavLink
                                key={tab.id}
                                to={tab.id === "DischargeProcessDoctor" ? `/doctor/${tab.id}/current` : `/doctor/${tab.id}`}
                                className={`
